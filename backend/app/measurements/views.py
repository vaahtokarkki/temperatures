...
from .utils import COLORS, get_minutes, get_night_low_measurements, save_measurement, authorized, get_measurements_from_now
...
def create_measurement(request):
    ...
    If server configuration includes API_TOKEN environment variable, it must be included in
    authorazion header. See more about authentication from documentation.
...
def graphs(request):
    ...
    for measurement in measurements_temperature:
        ...
        formated_timestamp = measurement.timestamp.timestamp()
        items.append({
